...
from .generated_objects import TwoCube

class PandaCL(RobotEnv):
    """
    ...
    def __init__(
        ...
        self.goal_locations = [
            [0.07, -0.08], [0.07, 0.08], [0.23, -0.08], [0.23, 0.08]
        ]
    ...
    def reward(self, action=None):
        ...
        if self.reward_shaping:
            # reaching reward
            # right_pos = self.sim.data.site_xpos[self.right_id]
            
            # # Gripper pos
            # gripper_site_pos = self.sim.data.site_xpos[self.robots[0].eef_site_id]

            # # xy dist
            # diff = np.abs(right_pos[0:2] - gripper_site_pos[0:2])

            # # tolerance in y direction
            # diff = np.maximum(0, diff - [0.08, 0.08])
            # xy_dist = np.linalg.norm(diff)
            # reaching_reward = 1 - np.tanh(10.0 * xy_dist)
            ...
            reward -= 0.25 * np.linalg.norm(action[:2])
    ...
    def _load_model(self):
        ...
        box = TwoCube("box", density_left=self.density[0], density_right=self.density[1])
    ...
    def _get_reference(self):
        ...
        self.handle_1_body_id = self.sim.model.body_name2id("handle_1")
        self.handle_2_body_id = self.sim.model.body_name2id("handle_2")
        self.center_id = self.sim.model.site_name2id("pot_center")
        self.left_id = self.sim.model.site_name2id("pot_left")
        self.right_id= self.sim.model.site_name2id("pot_right")
        self.corner_ids = [self.sim.model.site_name2id(f"corner_{i}") for i in range(1, 5)]
    ...
    def _reset_internal(self):
        ...
        obj_pos, obj_quat = np.ones((1,3)),np.ones((1,4))
        obj_pos[0] = [-0.1, 0, 0.84029956]
        obj_quat[0] = [0, 0, 0, 0]
    ...
    def reset(self):
        ...
        reset_pos = np.array([-0.05, 0., 0.])
        direction = obs['robot0_robot-state'] + reset_pos
        for i in range(20):
            obs, _, done, _ = self.step(direction * 10, move_z=True)
            direction = obs['robot0_robot-state'] + reset_pos
    ...
    def _get_observation(self):
        ...
        if self.use_object_obs:
            ...
            if self.include_z_obs:
                ...
                right_pos = np.array(self.sim.data.site_xpos[self.right_id])
                gripper_site_pos = np.array(self.sim.data.site_xpos[self.robots[0].eef_site_id])
            else:
                right_pos = np.array(self.sim.data.site_xpos[self.right_id][:2])
                gripper_site_pos = np.array(self.sim.data.site_xpos[self.robots[0].eef_site_id][:2])

            relative_right_pos = right_pos - gripper_site_pos

            # position of corners
            corners = np.concatenate([
                np.array(self.sim.data.site_xpos[self.corner_ids[0]][:2]),
                np.array(self.sim.data.site_xpos[self.corner_ids[1]][:2]),
                np.array(self.sim.data.site_xpos[self.corner_ids[2]][:2]),
                np.array(self.sim.data.site_xpos[self.corner_ids[3]][:2]),
            ])

        dii = {"robot0_robot-state": relative_right_pos, "object-state": corners}
        return dii
