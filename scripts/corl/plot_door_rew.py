...
for t in range(tasks):
    ...
    plt.ylim([-0.4, 2.2]) 
    fig.axes[t].yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
...
#plt.ylim([-0.4, 2.3])
fig.axes[5].yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
...
plt.subplots_adjust(wspace=0.10, hspace=0.1, left=0.06, right=0.985, top=0.91, bottom=0.1)

#plt.tight_layout()
filename = osp.join(base, f'{env}_reward_icra.pdf')
